...
// ══ Auth State Listener — يعالج OAuth redirect بشكل موثوق ══
_supabase.auth.onAuthStateChange(async function (event, session) {
  // نتجاهل الأحداث إذا المستخدم محمّل مسبقاً
  if (user) return;
  ...
  if (profile) {
    ...
    // مستخدم جديد أو حُذفت بياناته — أظهر modal إدخال البيانات
    _googlePendingSession = session;
    const gsNameEl = document.getElementById("gsName");
    if (gsNameEl) {
      gsNameEl.value =
...
async function executeDeleteUserData() {
  ...
  try {
    ...
    if (error) {
      if (typeof toast === "function")
        toast("حدث خطأ أثناء الحذف: " + error.message, "error");
      closeDeleteConfirm();
      return;
    }
