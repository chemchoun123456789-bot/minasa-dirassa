...
function tick() {
  if (timeLeft > 0) {
    ...
    if (pomoMode === "study" && user) {
      ...
      if (timeLeft % 30 === 0) saveUser();
      if (timeLeft % 60 === 0) renderStats();
    }
  } else {
    handleModeSwitch();
  }
...
function updateDashHeader() {
  ...
  var todaySec = (user.data.studyLog && user.data.studyLog[today]) || 0;
  var h = Math.floor(todaySec / 3600),
    m = Math.floor((todaySec % 3600) / 60);
  dhTodayTime.textContent =
    todaySec === 0 ? "0د" : h > 0 ? h + "س " + (m > 0 ? m + "د" : "") : m + "د";
