...
function togglePomoSettings() {
  ...
  if (isOpening) {
    // حفظ لقطة من القيم الحالية عند فتح الإعدادات
    ...
    // مزامنة حقول الإدخال مع القيم الفعلية الحالية
    var studyInp = document.getElementById("studyInp");
    var breakInp = document.getElementById("breakInp");
    var sessionInp = document.getElementById("sessionInp");
    var longBreakInp = document.getElementById("longBreakInp");
    if (studyInp) studyInp.value = config.study;
    if (breakInp) breakInp.value = config.break;
    if (sessionInp) sessionInp.value = config.sessions;
    if (longBreakInp) longBreakInp.value = config.longBreak;
  }
}

function cancelPomoSettings() {
  if (_pomoSettingsSnapshot) {
    // استعادة إعداد صوت الساعة ومسح localStorage
    ...
    // استعادة قيم حقول الإدخال
    var studyInp = document.getElementById("studyInp");
    var breakInp = document.getElementById("breakInp");
    var sessionInp = document.getElementById("sessionInp");
    var longBreakInp = document.getElementById("longBreakInp");
    if (studyInp) studyInp.value = _pomoSettingsSnapshot.study;
    if (breakInp) breakInp.value = _pomoSettingsSnapshot.breakTime;
    if (sessionInp) sessionInp.value = _pomoSettingsSnapshot.sessions;
    if (longBreakInp) longBreakInp.value = _pomoSettingsSnapshot.longBreak;
  ...
  Object.keys(bySubject).forEach(function (sub) {
    ...
    arr.forEach(function (g) {
      ...
      html +=
        ...
        g.idx +
        ')" class="glass-del-btn" aria-label="حذف" style="margin-right:4px;"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><path d="M18 6L6 18M6 6l12 12"/></svg></button>' +
        "</div></div>";
    });
    html += "</div></div>";
