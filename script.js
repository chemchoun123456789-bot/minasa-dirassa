...
// [SECURITY] قائمة كلمات المرور الشائعة المحظورة
...
// [SECURITY] التحقق من قوة كلمة المرور
...
// [SECURITY] Rate limiting على تسجيل الدخول
...
async function submitGoogleProfile() {
  ...
  if (name.length > 60)
    return toast("الاسم طويل جداً (60 حرف كحد أقصى)", "error");
  if (!school) return toast("يرجى إدخال اسم المدرسة أو الجامعة", "error");
  if (school.length > 80)
    return toast("اسم المدرسة طويل جداً (80 حرف كحد أقصى)", "error");
  const _passErr = validatePassSecurity(pass);
  if (_passErr) return toast(_passErr, "error");
...
async function handleAuth(type) {
  if (type === "register") {
    ...
    if (school.length > 80)
      return toast("اسم المدرسة طويل جداً (80 حرف كحد أقصى)", "error");
    const _passErrReg = validatePassSecurity(pass);
    if (_passErrReg) return toast(_passErrReg, "error");
    ...
    if (authError) {
      // [SECURITY] رسالة عامة — لا تكشف إذا كان الإيميل مسجلاً أم لا
      console.error("Register error:", authError.message);
      return toast(
        "تعذّر إنشاء الحساب، تحقق من البيانات وأعد المحاولة",
        "error",
      );
    }
    ...
    if (_loginLocked)
      return toast("محاولات كثيرة — انتظر 30 ثانية ثم أعد المحاولة", "error");
    ...
    if (authError) {
      // [SECURITY] لا نكشف تفاصيل الخطأ للمستخدم
      ...
      if (_loginAttempts >= 5) {
        ...
        return toast("5 محاولات فاشلة — الحساب مقفل 30 ثانية", "error");
      }
...
function escHtml(str) {
  ...
  // [SECURITY] sanitizeUrl — تمنع javascript: و data: داخل href
  function sanitizeUrl(url) {
    const s = String(url || "").trim();
    if (/^javascript:/i.test(s) || /^data:/i.test(s) || /^vbscript:/i.test(s))
      return "#";
    if (!/^https?:\/\//i.test(s) && s !== "#") return "#";
    return s;
  }
}
...
function addResource() {
  ...
  if (title.length > 120) return toast("عنوان الرابط طويل جداً", "error");
  if (url.length > 500) return toast("الرابط طويل جداً", "error");
