(function () {
  ...
  /* ---------------------------------------------------------------
         2. SECTION LIST — with Arabic names + SVG icons for tab bar
         IMPROVEMENT 1: Added `name` (Arabic) and `icon` (inline SVG)
         to each section so buildDots() can render a proper bottom
         tab bar on mobile instead of bare 6px dots.
      --------------------------------------------------------------- */
  var SECTIONS = [
    {
      id: "sec-schedule",
  ...
  /* ---------------------------------------------------------------
         3. BUILD SIDEBAR DOTS / BOTTOM TAB BAR
         IMPROVEMENT 1: Each .sdot now contains:
           - .sdot-icon  (SVG, shown on mobile via mobile.css)
           - .sdot-name  (Arabic label, shown on mobile via mobile.css)
         Desktop: child elements hidden → dots look exactly as before.
         Mobile:  child elements shown → full tab bar.
      --------------------------------------------------------------- */
  ...
  function buildDots() {
    ...
    SECTIONS.forEach(function (s, i) {
      ...
      // Icon wrapper — hidden on desktop, shown on mobile via CSS
      var iconWrap = document.createElement("div");
      iconWrap.className = "sdot-icon";
      iconWrap.innerHTML = s.icon;

      // Name label — hidden on desktop, shown on mobile via CSS
  ...
  /* ---------------------------------------------------------------
         ...
         IMPROVEMENT 9: swipeThreshold is now relative to screen width
         (12% of viewport, min 40px) instead of a fixed 60px.
         This makes swipes feel natural on narrow phones (320px).
      --------------------------------------------------------------- */
  var touchStartX = 0;
  var touchStartY = 0;
  var swipeThreshold = 50; // default; recalculated on each touchstart
  ...
  document.addEventListener(
    ...
    function (e) {
      ...
      swipeActive = true;
      // IMPROVEMENT 9 — recalculate on every touch so orientation changes
      // and zoom levels are handled correctly.
  ...
  /* ---------------------------------------------------------------
         10. FLOATING ANIMATION on pomo timer
      --------------------------------------------------------------- */
  if (!reducedMotion) {
