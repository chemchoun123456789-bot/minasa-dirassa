(function () {
  ...
  var SECTIONS = [
    ...
    },
    {
      ...
      label: "📆",
      name: "الفروض",
      icon: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="2" y="3" width="14" height="13" rx="2" ry="2"/><line x1="6" y1="1" x2="6" y2="5"/><line x1="11" y1="1" x2="11" y2="5"/><line x1="2" y1="8" x2="16" y2="8"/><circle cx="18.5" cy="17.5" r="4.5"/><polyline points="18.5 15.5 18.5 17.5 20 17.5"/></svg>',
    },
  ];
  // نفس نقطة الكسر المستعملة في mobile.css (@media max-width: 768px)
  var MOBILE_BREAKPOINT = 768;
  function isMobileViewport() {
    return window.innerWidth <= MOBILE_BREAKPOINT;
  }
  // على البيسي: sec-notes و sec-resources يظهروا جنب لبعض في نفس الصف
  // (#sec-info-row)، فيدمجوا في محطة تنقل واحدة بدل اثنين.
  // على الهاتف: يظهروا فوق بعض كقسمين منفصلين (نفس SECTIONS الأصلية، بلا تغيير).
  function getActiveSections() {
    if (isMobileViewport()) return SECTIONS;
    var merged = [];
    SECTIONS.forEach(function (s) {
      if (s.id === "sec-resources") return;
      if (s.id === "sec-notes") {
        merged.push({
          id: "sec-info-row",
          label: s.label,
  ...
  function buildDots() {
    ...
    getActiveSections().forEach(function (s, i) {
  ...
  function updateDotUI(idx) {
    ...
    if (arrNext)
      arrNext.classList.toggle(
        "disabled",
        idx === getActiveSections().length - 1,
      );
  }
  function showNavUI() {
  ...
  function navigateTo(idx) {
    ...
    var sectionId = activeSections[idx].id;
  ...
  function scrollSpy() {
    // إذا كنا فعلاً فوق الصفحة، نرجعو مباشرة للقسم الأول بلا ما نقيسو
    // أي عنصر — يمنع أي قراءة خاطئة (rect.top) وقت ما الـ layout لسه
    // ما استقرش (صور/عناصر reveal لسه كيرندريو) من كون تختار قسم غلط
    var scrollY = window.scrollY || document.documentElement.scrollTop || 0;
    if (scrollY < 4) {
      if (currentSectionIdx !== 0) updateDotUI(0);
      return;
    }
    ...
    getActiveSections().forEach(function (s, i) {
      ...
      // نتجاهل أي عنصر ارتفاعه صفر (لسه ما رندرياش بشكل صحيح) باش ما
      // يأثرش على حساب القسم النشط
  ...
  var _wasMobileViewport = isMobileViewport();
  window.addEventListener("resize", function () {
    var nowMobile = isMobileViewport();
    if (nowMobile === _wasMobileViewport) return;
