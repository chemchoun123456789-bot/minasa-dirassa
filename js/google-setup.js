...
/* ============================================================
   أخطاء الحقول المضمّنة — نسخة طبق الأصل من script.js
   ============================================================ */
...
var _gsSession = null;
var _gsFormSubmitted = false;

// طبقة تحقق إضافية من طرف السيرفر (Edge Function) قبل حفظ الملف الشخصي.
// إذا تعذر الوصول لها (شبكة/عدم نشرها بعد)، نكمل بالتحقق المحلي فقط.
...
async function submitGoogleProfile() {
  ...
  if (nameErr) errors.gsName = nameErr;
  if (!school) errors.gsSchool = "أدخل اسم المدرسة أو الجامعة";
  else {
    var schoolErr = validateInstitutionName(school);
    if (schoolErr) errors.gsSchool = schoolErr;
  }
  var passErr = validatePassSecurity(pass);
  if (passErr) errors.gsPass = passErr;
  ...
  if (!session) {
    toast("انتهت صلاحية الجلسة، جاري إعادة تحويلك...", "error");
    setTimeout(function () {
  ...
  if (serverCheck && serverCheck.ok === false) {
    ...
    var gsIdMap = { name: "gsName", school: "gsSchool", password: "gsPass" };
  ...
  // رسالة انتقالية محايدة فقط — رسالة الترحيب الحقيقية توري مرة وحدة
  // بعد الوصول لـ index.html (راجع _handleGoogleSession فـ script.js)
  sessionStorage.setItem("dirasa_fresh_signup", "1");
  toast("تم إنشاء حسابك، جاري تحويلك...", "info");
  _gsFormSubmitted = true;
  setTimeout(function () {
    window.location.href = "/";
  }, 900);
}
...
(async function initGoogleSetupPage() {
  ...
  hideAuthGateOverlay();
  // يمنع الخروج العرضي (تبديل الرابط، إغلاق التبويبة...) قبل ما يكمل
  // بياناته أو يضغط "إلغاء وتسجيل الخروج" — المتصفح كيعرض تنبيهه الافتراضي
  window.addEventListener("beforeunload", function (e) {
    if (_gsFormSubmitted) return;
    e.preventDefault();
    e.returnValue = "";
  });
})();

// لو الصفحة رجعت من bfcache (زر "رجوع" فالمتصفح) نعاود تحميلها كاملة —
// باش initGoogleSetupPage() يخدم من جديد ديما، وما يبقاش يبان أي DOM
// مجمّد من قبل (فورم فارغ، أو حالة قديمة) بلا ما يتعاود التحقق من السيرفر
