...
// ══════════════════════════════════════════════════════════════
// استمرارية عدادات القفل (OTP + دخول) عبر localStorage
// ══════════════════════════════════════════════════════════════
// هذا تحسين UX/ردع خفيف بس، مش حماية أمنية حقيقية: أي سكريبت يقدر
// ينادي Supabase API مباشرة بلا ما يمر من هاذ الكود إطلاقاً، أو
// يفتح المتصفح فوضع خاص (Incognito) بلا localStorage قديم. الحماية
// الحقيقية لازم تكون من طرف Supabase (Authentication → Rate Limits)
// + CAPTCHA. هاذي بس تمنع إعادة تحميل الصفحة العادية من تصفير القفل.
function loadLockState(key) {
  try {
...
function saveLockState(key, attempts, lockUntil) {
  try {
    ...
    localStorage.setItem(
      key,
      JSON.stringify({ attempts: attempts, lockUntil: lockUntil }),
    );
  } catch (err) {
...
var _otpEmail = "";
var _otpLockState = loadLockState("dirasa_otp_lock");
var _otpAttempts = _otpLockState.attempts;
var _otpLockUntil = _otpLockState.lockUntil;
...
function showOtpScreen(email) {
  ...
  _otpAttempts = 0;
  _otpLockUntil = 0;
  saveLockState("dirasa_otp_lock", 0, 0);
...
async function submitOtp() {
  ...
  if (res.error) {
    _otpAttempts++;
    saveLockState("dirasa_otp_lock", _otpAttempts, _otpLockUntil);
    ...
    if (_otpAttempts >= 5) {
      ...
      _otpAttempts = 0;
      saveLockState("dirasa_otp_lock", 0, _otpLockUntil);
      clearOtpBoxes();
      document.getElementById("otpAttemptsMsg").textContent =
        "محاولات كثيرة، انتظر 30 ثانية وحاول من جديد";
...
var _loginLockState = loadLockState("dirasa_login_lock");
var _loginAttempts = _loginLockState.attempts;
var _loginLockUntil = _loginLockState.lockUntil;
var _loginLockInterval = null;
// إذا كان فيه قفل نشط محفوظ من قبل (المستخدم عاود تحميل الصفحة وهو
// مقفول)، وريه البانر فوراً بلا ما ننتظر يدوس "دخول" باش يكتشف
if (_loginLockUntil > Date.now()) {
  document.addEventListener("DOMContentLoaded", function () {
    showLoginLockBanner(_loginLockUntil);
  });
}

// بانر ثابت لقفل محاولات الدخول — يبقى ظاهراً طول مدة القفل الحقيقية
// عكس toast اللي يختفي بعد مدة ثابتة (10 ثواني) حتى لو القفل أطول
function showLoginLockBanner(untilTs) {
  ...
  function tick() {
    ...
    if (remaining <= 0) {
      ...
      saveLockState("dirasa_login_lock", 0, 0);
      return;
    }
...
async function handleAuth(type) {
  if (type === "register") {
    ...
    if (authError) {
      _loginAttempts++;
      saveLockState("dirasa_login_lock", _loginAttempts, _loginLockUntil);
      if (_loginAttempts >= 5) {
        _loginLockUntil = Date.now() + 30000;
        _loginAttempts = 0;
        saveLockState("dirasa_login_lock", 0, _loginLockUntil);
        showLoginLockBanner(_loginLockUntil);
        return;
      }
