...
function loadCachedLock(key) {
  try {
    ...
    if (!raw) return { scope: null, lockUntil: 0 };
    ...
    return { scope: null, lockUntil: 0 };
  }
}

function saveCachedLock(key, scope, lockUntil) {
  try {
    if (!lockUntil) {
    ...
    localStorage.setItem(
      key,
      JSON.stringify({ scope: scope, lockUntil: lockUntil }),
    );
  } catch (err) {
...
function applyLoginLock(scope, remainingSeconds) {
  ...
  saveCachedLock("dirasa_login_lock", scope, untilTs);
  if (scope === "ip") {
...
// حتى لو القفل أطول
function showLoginLockBanner(untilTs) {
  ...
  // شريط التقدم: انتقال CSS وحيد من scaleX(1) لـ scaleX(0) بالضبط
  // على مدة الثواني المتبقية، بلا حاجة نحدّثو كل ثانية من JS
  if (progressEl) {
    var totalRemaining = Math.max(0, (untilTs - Date.now()) / 1000);
    progressEl.style.transition = "none";
    progressEl.style.transform = "scaleX(1)";
    void progressEl.offsetWidth; // إجبار إعادة الرسم قبل تفعيل الانتقال
    progressEl.style.transition = "transform " + totalRemaining + "s linear";
