...
async function cancelGoogleSetup() {
  ...
  document.body.classList.remove("gs-modal-open");
  _googlePendingSession = null;
  // logout() يدير reset كامل وموثوق (لوحة التحكم، الشريط العلوي/السفلي،
  // المؤقتات، الأصوات...) — نعيد استعماله هنا بدل ما نكرر جزء ناقص منه
  await logout();
}
...
(async function () {
  ...
  if (user) return; // تفادي التكرار — المستخدم محمّل بالفعل من الفحص الآخر
  if (sessionData && sessionData.session) {
    const uid = sessionData.session.user.id;
    const email = sessionData.session.user.email;
    const profile = await dbLoadUser(uid);
    if (user) return; // تفادي التكرار — قد يكون تحمّل أثناء انتظار البروفايل
    if (profile) {
      user = {
        id: uid,
        name: profile.name,
