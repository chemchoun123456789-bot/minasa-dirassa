...
// آخر قفل IP وريناه للمستخدم (اهتزاز + رسالة تحت كلمة السر) — يمنع
// تكرار الاهتزاز والرسالة كل ما يدوس زر الدخول وهو مازال مقفول
// بنفس القفل، توريه مرة وحدة بس لكل قفل جديد
var _ipLockNoticeUntil = 0;
...
function applyLoginLock(scope, remainingSeconds) {
  ...
  if (scope === "ip") {
    ...
    // نوريها مرة وحدة بس لكل قفل (مش فكل ضغطة زر وهو مازال مقفول
    // بنفس القفل)
    if (Math.abs(untilTs - _ipLockNoticeUntil) > 2000) {
      _ipLockNoticeUntil = untilTs;
      showFieldError(
        "pass",
...
function showLoginLockBanner(untilTs, startedAt) {
  ...
  // نثبت الشريط مباشرة في مكانه الصح بلا أي انتقال/سلايد — يبان
  // فورا فنسبته الحقيقية حتى أول مرة (بعد refresh مثلاً)، وبعدها
  // يتحدّث كل ثانية لنفس المكان الصح، بلا تكرار "الامتلاء" من جديد
