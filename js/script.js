...
// ══════════════════════════════════════════════════════════════
// حارس محاولات login/otp — الحماية الحقيقية من طرف السيرفر
// ══════════════════════════════════════════════════════════════
// المصدر الوحيد للحقيقة هو Postgres (سلم مستقل لكل من email/IP،
// محسوب عبر auth-attempt-guard Edge Function). localStorage هنا بس
// كاش عرض — يخلي القفل يبان فوراً عند إعادة تحميل الصفحة بلا ما
// ننتظر رد سيرفر جديد؛ ما يعتبرش مصدر موثوق ولا يُستعمل فأي قرار
// أمني (القرار الحقيقي ديماً من رد auth-attempt-guard).
function loadCachedLock(key) {
  try {
    var raw = localStorage.getItem(key);
    if (!raw) return { scope: null, lockUntil: 0, startedAt: 0 };
    ...
    return { scope: null, lockUntil: 0, startedAt: 0 };
  }
}

function saveCachedLock(key, scope, lockUntil, startedAt) {
  try {
    if (!lockUntil) {
    ...
    // localStorage قد يكون معطل (وضع خاص صارم مثلاً) — نكمل بالحالة
    // فالذاكرة بس، بلا ما نكسر الصفحة
  }
}

// دقائق:ثواني للعرض الثابت (قفل الـ IP) — بلا عداد متحرك، كيما
// طلب صاحب المشروع.
function formatMinSec(totalSeconds) {
  var s = Math.max(0, Math.ceil(totalSeconds || 0));
...
// يتأكد من السيرفر قبل أي محاولة دخول/otp فعلية. فشل شبكة/سيرفر =
// نخلي المحاولة تعدي (fail-open) — نفس فلسفة validateRegistrationServerSide
// الموجودة أصلاً فالمشروع؛ الحماية الأخيرة ضد إساءة الاستعمال
// الحقيقية تبقى Supabase Auth Rate Limits، مستقلة تماماً عن هاذي.
async function checkAuthAttempt(actionType, email) {
  try {
    const res = await fetch(
      SUPABASE_URL + "/functions/v1/auth-attempt-guard",
      {
        method: "POST",
        headers: {
    ...
    console.warn(
      "checkAuthAttempt: تعذر التحقق من السيرفر، تابعنا محلياً",
      err,
    );
    return { allowed: true };
  }
...
async function reportAuthOutcome(actionType, email, success) {
  try {
    const res = await fetch(
      SUPABASE_URL + "/functions/v1/auth-attempt-guard",
      {
    ...
    console.warn("reportAuthOutcome: تعذر التبليغ للسيرفر", err);
    return { locked: false };
  }
}
...
var _otpEmail = "";
var _otpLockCache = loadCachedLock("dirasa_otp_lock");
var _otpLockUntil = _otpLockCache.lockUntil;
...
function showOtpScreen(email) {
  _otpEmail = email;
  _otpLockUntil = 0;
  saveCachedLock("dirasa_otp_lock", null, 0);
...
function _startOtpLockTicker(lockScope) {
  var lockInterval = setInterval(function () {
    ...
    if (remaining <= 0) {
      ...
      document.getElementById("otpAttemptsMsg").textContent =
        lockScope === "ip"
          ? "رصدنا نشاط كثير من نفس الشبكة، حاول بعد " + remaining + " ثانية"
          : "تجاوزت الحد المسموح، انتظر " + remaining + " ثانية";
    }
  }, 1000);
}
...
async function submitOtp() {
  if (Date.now() < _otpLockUntil) {
    ...
    document.getElementById("otpAttemptsMsg").textContent =
      (_otpLockCache.scope === "ip"
        ? "رصدنا نشاط كثير من نفس الشبكة، حاول بعد "
        : "انتظر ") + secs + " ثانية" + (_otpLockCache.scope === "ip" ? "" : " قبل المحاولة مجدداً");
    return;
  }
  ...
  if (code.length < 6) {
    document.getElementById("otpAttemptsMsg").textContent =
      "أدخل الكود كاملاً (6 أرقام)";
    return;
  }

  // تحقق حقيقي من السيرفر قبل أي محاولة فعلية — سلم الإيميل + سلم
  // الـ IP، الاثنين مستقلين (راجع auth-attempt-guard)
  var check = await checkAuthAttempt("otp", _otpEmail);
  if (!check.allowed) {
    _otpLockUntil = Date.now() + (check.remaining_seconds || 0) * 1000;
    _otpLockCache = { scope: check.lock_scope, lockUntil: _otpLockUntil };
    saveCachedLock("dirasa_otp_lock", check.lock_scope, _otpLockUntil);
    _startOtpLockTicker(check.lock_scope);
    return;
  }
  ...
  if (res.error) {
    ...
    var outcome = await reportAuthOutcome("otp", _otpEmail, false);

    if (outcome.locked) {
      _otpLockUntil = Date.now() + (outcome.remaining_seconds || 0) * 1000;
      _otpLockCache = { scope: outcome.lock_scope, lockUntil: _otpLockUntil };
      saveCachedLock("dirasa_otp_lock", outcome.lock_scope, _otpLockUntil);
      clearOtpBoxes();
      document.getElementById("otpAttemptsMsg").textContent =
        outcome.lock_scope === "ip"
          ? "رصدنا نشاط كثير من نفس الشبكة، حاول بعد " +
            (outcome.remaining_seconds || 0) +
            " ثانية"
          : "محاولات كثيرة، انتظر " +
            (outcome.remaining_seconds || 0) +
            " ثانية وحاول من جديد";
      _startOtpLockTicker(outcome.lock_scope);
      return;
    }

    var remaining =
      outcome.attempts_remaining != null ? outcome.attempts_remaining : "؟";
    document.getElementById("otpAttemptsMsg").textContent =
      "الكود غير صحيح، بقيت لك " + remaining + " محاولة";
    clearOtpBoxes();
    boxes[0].focus();
    return;
  }

  // نجح التحقق — نبلغ السيرفر (يصفّر سلم الإيميل بس) ونصفّر العرض المحلي
  reportAuthOutcome("otp", _otpEmail, true);
  saveCachedLock("dirasa_otp_lock", null, 0);
  ...
  var profileErr = await _supabase.from("user_profiles").upsert({
    id: uid,
    name: pending.name,
...
async function resendOtp() {
  ...
  // تفعيل عداد 5 دقائق. ملاحظة: هذا يصفّر العرض المحلي بس (رسالة
  // "بقيت لك X محاولة")، ما يصفّرش سلم المحاولات فالسيرفر — عمداً،
  // باش سبام "إعادة الإرسال" ما يتقدرش يستعمل كطريقة لتصفير رصيد
  // المحاولات فـ OTP (لو صفّرناه هنا، كان يبقى بايباس سهل للسلم).
...
function openNoCodeModal() {
  document.getElementById("noCodeModal").style.display = "flex";
}

function closeNoCodeModal() {
  document.getElementById("noCodeModal").style.display = "none";
}
...
var MAX_PASS_LEN = 64;
var MAX_NAME_LEN = 60;
var MAX_SCHOOL_LEN = 90;
var MAX_EMAIL_LEN = 120;

function validatePassSecurity(pass) {
  if (pass.length < 8) return "كلمة المرور قصيرة، يجب أن تكون 8 أحرف على الأقل";
  if (pass.length > MAX_PASS_LEN)
    return "كلمة المرور طويلة جداً، الحد الأقصى " + MAX_PASS_LEN + " حرفاً";
...
function validateFullName(name) {
  ...
  if (!trimmed) return "أدخل اسمك الكامل";
  if (trimmed.length > MAX_NAME_LEN)
    return "الاسم طويل جداً، الحد الأقصى " + MAX_NAME_LEN + " حرفاً";
  return null;
}
...
function validateInstitutionName(school) {
  ...
  if (trimmed.length > MAX_SCHOOL_LEN)
    return "اسم المؤسسة طويل جداً، الحد الأقصى " + MAX_SCHOOL_LEN + " حرفاً";
...
function resolveAudioPath(file, kind) {
  ...
  if (kind === "focus") {
    return "assets/sounds/study-focus-sounds/" + file;
  }
  if (kind === "clock") {
    return "assets/sounds/" + file;
  }
  return "assets/sounds/alarm-sounds/" + file;
}

window.resolveAudioPath = resolveAudioPath;
...
function _playAlarmFile(wasStudy) {
  ...
  try {
    var a = new Audio(resolveAudioPath(file, "alarm"));
    a.volume = vol;
...
function _startClockTick() {
  ...
  const audio = new Audio(
    resolveAudioPath("clock_ticking_sound_effect_-_SILHOUETTE_GEARS.mp3", "clock"),
  );
  audio.loop = true;
  audio.volume = 0;
...
// ══════════════════════════════════════════════
// أخطاء الحقول المضمّنة (Inline Field Errors)
// تُستعمل بدل toast لأي خطأ مرتبط بحقل إدخال محدد
// ══════════════════════════════════════════════
function showFieldError(inputId, message) {
  ...
  if (inp) {
    ...
    void inp.offsetWidth; // إعادة تشغيل أنيميشن الاهتزاز حتى لو الخطأ نفسه متكرر
  ...
  // تلميح "تأكد من صحة بريدك الإلكتروني" ينخفي وقت ظهور خطأ الإيميل —
  // قرار مخصص لهاذ الحقل بالذات، مش آلية عامة لكل الحقول (راجع تعليق
  // .field-hint فـ style.css). إذا حبيت تزيد حقل آخر، زيد شرط مشابه هنا.
  if (inputId === "regEmail") {
    var regEmailHint = document.getElementById("regEmailHint");
    if (regEmailHint) regEmailHint.classList.add("hint-hidden");
  }
}
...
function clearFieldError(inputId) {
  ...
  if (inputId === "regEmail") {
    var regEmailHint = document.getElementById("regEmailHint");
    if (regEmailHint) regEmailHint.classList.remove("hint-hidden");
  }
}

// اهتزاز الحقل بلا رسالة تحته — يُستعمل حين خطأ واحد مشترك بين حقلين
// (مثال: بريد/كلمة مرور خاطئين، بلا تحديد أي واحد بالضبط لأسباب أمنية)
function shakeFieldOnly(inputId) {
  var inp = document.getElementById(inputId);
  if (!inp) return;
  inp.classList.remove("input-error");
...
document.addEventListener("keydown", (e) => {
  if (e.key === "Escape") {
    ...
    const ncModal = document.getElementById("noCodeModal");
    if (ncModal.style.display === "flex") closeNoCodeModal();
...
async function _handleGoogleSession(session) {
  ...
  try {
    ...
    if (user) return; // تفادي التكرار — قد يكون تحمّل من مسار آخر بالتوازي
    ...
    if (profile && profile.school) {
      ...
      // لو جاي تواً من صفحة إكمال بيانات Google، خلي رسالة الترحيب توري
      // مرة وحدة بس هنا (وليس مرتين: وحدة فـ google-setup ووحدة هنا)
      if (sessionStorage.getItem("dirasa_fresh_signup") === "1") {
        sessionStorage.removeItem("dirasa_fresh_signup");
        toast(`تم إنشاء حسابك، أهلاً بك يا ${user.name}`, "success");
      } else {
        toast(`أهلاً ${user.name}! 👋`, "success");
      }
      initDashboard();
    } else {
      // حساب جديد أو بياناته محذوفة — ننقله لصفحة إعداد مستقلة تماماً
      // (بلا أي كود من لوحة التحكم/السايدبار/الأونبوردينغ)
      // ملاحظة: ما نخبيوش overlay هنا بالنية — يبقى ظاهر حتى يتم الانتقال
      // لصفحة google-setup لي عندها نفس overlay، باش الانتقال يكون بلا فلاش
      window.location.href = "/pages/google-setup";
    }
  } catch (err) {
...
// إعادة ضبط زر "الدخول بواسطة Google" لحالته الافتراضية (غير محمّل/بلا سبينر)
// موحّدة هنا حتى تُستدعى من أي مكان يحتاج إعادة الضبط بدل تكرار نفس الأسطر
function resetGoogleSignInButton() {
  var btn = document.getElementById("googleSignInBtn");
  var inner = document.getElementById("googleBtnInner");
  var spinner = document.getElementById("googleSpinner");
  if (btn) btn.disabled = false;
  if (inner) inner.style.display = "flex";
  if (spinner) spinner.style.display = "none";
}

// تخفي شاشة التحميل (Auth Gate) — تُستدعى فقط بعد ما تتحدد الوجهة النهائية
// (داشبورد/صفحة دخول). ما تُستدعاش قبل redirect لـ google-setup باش يبقى
// الانتقال بلا فلاش.
function hideAuthGateOverlay() {
  ...
  // نشيل قفل السكرول باش الصفحة (authArea/dashboard) ترجع قابلة للسكرول عادي
...
(async function checkGoogleSession() {
  try {
    ...
    if (!session) {
      // لا توجد جلسة نشطة — لو الزر باقي بحالة تحميل من محاولة سابقة
      // (نادراً، فحالة عدم استخدام المتصفح لـ bfcache)، نعيد ضبطه احتياطاً
    ...
    if (provider === "email") {
      ...
      if (profile && profile.__loadError) {
        ...
        toast("حسابك غير مكتمل، سجّل من جديد", "error");
        hideAuthGateOverlay();
      }
    } else {
      // provider غير معروف → onAuthStateChange سيتعامل معه لو Google،
      // وإلا نكشف authArea باش المستخدم ما يبقاش عالق ورا شاشة التحميل
      hideAuthGateOverlay();
    }
  } catch (err) {
...
// نفس مبدأ resetGoogleSignInButton بس للزر الجديد فمودال noCodeModal —
// عمداً مفصولة باش handleGoogleSignIn() تبقى بلا أي تعديل
function resetNoCodeGoogleBtn() {
  var btn = document.getElementById("noCodeGoogleBtn");
  var inner = document.getElementById("noCodeGoogleBtnInner");
  var spinner = document.getElementById("noCodeGoogleSpinner");
  if (btn) btn.disabled = false;
  if (inner) inner.style.display = "flex";
  if (spinner) spinner.style.display = "none";
}

// Wrapper حوالي handleGoogleSignIn() يدير loading state على زر المودال
// (handleGoogleSignIn() نفسها تدير الحالة على googleSignInBtn الأصلي بس)
async function handleNoCodeGoogleSignIn() {
  var btn = document.getElementById("noCodeGoogleBtn");
  var inner = document.getElementById("noCodeGoogleBtnInner");
  var spinner = document.getElementById("noCodeGoogleSpinner");
  btn.disabled = true;
  ...
  try {
    ...
    resetNoCodeGoogleBtn();
  }
}

// الإصلاح الرئيسي: الزر كان يبقى عالقاً بحالة "تحميل" لو المستخدم ضغط
// "رجوع" فالمتصفح وهو فصفحة Google — لأن المتصفح يرجّع الصفحة من bfcache
// (نسخة مجمّدة فالذاكرة) بلا إعادة تحميل حقيقية، فـ الكود ما يعرفش إنه رجع.
// نستمع لـ pageshow ونتأكد عبر event.persisted (أو نوع التنقل احتياطاً)،
// ونعيد ضبط الزر بشكل غير مشروط فـ الحالتين.
window.addEventListener("pageshow", function (event) {
  var navEntries =
    typeof performance !== "undefined" && performance.getEntriesByType
      ? performance.getEntriesByType("navigation")
      : [];
  var isBackForward = navEntries[0] && navEntries[0].type === "back_forward";
...
var _loginLockCache = loadCachedLock("dirasa_login_lock");
var _loginLockUntil = _loginLockCache.lockUntil;
var _loginLockStartedAt = _loginLockCache.startedAt || 0;
var _loginLockInterval = null;
// آخر قفل بانر (قفل الإيميل) بدأنا نعرضه فهاذ التحميل للصفحة — يمنع
// إعادة تشغيل/تقطيع شريط التقدم والعداد كل ما يدوس المستخدم زر
// الدخول وهو مازال مقفول بنفس القفل (يخليهم يكملو عاديين بلا أي
// تأثير)، بلا ما يأثر على رسالة قفل الـ IP اللي تبقى تهتز وتتحدث
// بحرية كل ضغطة كيما باقي أخطاء الحقول
var _loginBannerShownFor = 0;
// إذا كان فيه قفل نشط محفوظ من قبل (المستخدم عاود تحميل الصفحة وهو
// مقفول)، وريه فوراً بلا ما ننتظر يدوس "دخول" باش يكتشف — البانر
// لقفل الإيميل، رسالة inline هادئة تحت كلمة السر لقفل الـ IP
if (_loginLockUntil > Date.now()) {
  document.addEventListener("DOMContentLoaded", function () {
    applyLoginLock(
      _loginLockCache.scope,
      (_loginLockUntil - Date.now()) / 1000,
    );
  });
}

// يطبّق حالة القفل حسب مصدره: قفل الإيميل (سلمه الخاص وصل لمرحلة
// القفل — المستخدم هو اللي دار المحاولات الفاشلة) يبان فالبانر
// الكبير المعتاد بعداد متحرك لايف. قفل الـ IP (نشاط كثير من نفس
// الشبكة، ممكن ميكونش هو شخصياً السبب) يبان كرسالة هادئة تحت حقل
// كلمة المرور (نفس ستايل خطأ الإيميل/الباسوورد)، بوقت ثابت بلا
// عداد متحرك — تفريق بصري متعمد بين "أنت السبب" و"شبكتك فيها نشاط".
function applyLoginLock(scope, remainingSeconds) {
  var untilTs = Date.now() + Math.max(0, remainingSeconds || 0) * 1000;
  // إذا هذا نفس القفل اللي عندنا أصلاً (فرق أقل من ثانيتين عن
  // untilTs المحفوظ) نحافظ على وقت بدايته الحقيقي (startedAt)، وإلا
  // (قفل جديد فعلاً) نبدأ عد جديد من الآن. هذا اللي يخلي شريط
  // التقدم يحسب نسبة صحيحة من مدة القفل الكاملة، بدل ما يرجع
  // "ممتلئ" فكل مرة يتحدّث فيها (زر الدخول يتدوس وهو مقفول، أو
  // refresh للصفحة)
  var startedAt =
    _loginLockStartedAt && Math.abs(untilTs - _loginLockUntil) < 2000
      ? _loginLockStartedAt
      : Date.now();
  _loginLockUntil = untilTs;
  _loginLockStartedAt = startedAt;
  saveCachedLock("dirasa_login_lock", scope, untilTs, startedAt);
  if (scope === "ip") {
    clearInterval(_loginLockInterval);
    showFieldError(
      "pass",
      "سجّلنا محاولات دخول كثيرة من نفس شبكتك خلال وقت قصير، فأوقفنا الدخول مؤقتاً لحماية الحسابات. جرّب بعد " +
        formatMinSec(remainingSeconds),
    );
  } else if (Math.abs(untilTs - _loginBannerShownFor) > 2000) {
    // قفل جديد فعلاً (أو أول ظهور فهاذ التحميل) — نبدأ عرض البانر
    // وعداده من جديد
    _loginBannerShownFor = untilTs;
    showLoginLockBanner(untilTs, startedAt);
  }
  // وإلا (نفس القفل اللي البانر شغال عليه أصلاً): ما نلمسوه — نخليه
  // يكمل عاديً بعداده وشريط تقدمه بلا ما يتقطع أو يتأثر بضغطة الزر
}

// بانر ثابت لقفل محاولات الدخول (قفل الإيميل بس) — يبقى ظاهراً طول
// مدة القفل الحقيقية عكس toast اللي يختفي بعد مدة ثابتة (10 ثواني)
// حتى لو القفل أطول
function showLoginLockBanner(untilTs, startedAt) {
  var banner = document.getElementById("loginLockBanner");
  var textEl = document.getElementById("loginLockText");
  var progressEl = document.getElementById("loginLockProgress");
  if (!banner || !textEl) return;
  banner.classList.add("show");

  // مدة القفل الكاملة (من بدايته الحقيقية لنهايته) — عليها نحسب
  // نسبة الشريط، مش على الوقت المتبقي لحاله (هذا اللي كان يخلي
  // الشريط يرجع "ممتلئ" كل ما يتحدّث القفل بنفس المدة)
  var totalDuration = Math.max(1, (untilTs - startedAt) / 1000);

  // أول ظهور (instant=true): نثبت الشريط مباشرة فمكانه الصح بلا أي
  // انتقال — مهم بعد refresh مثلاً، حتى ما يبانش يمتلئ ويسلايد من
  // جديد. بعدها كل تحديث (كل ثانية): انتقال CSS سلس لمدة ثانية
  // كاملة، فالشريط يتحرك بخط متواصل وناعم مش يقفز/يتقطع كل ثانية
  function setProgress(instant) {
    if (!progressEl) return;
    var remaining = Math.max(0, (untilTs - Date.now()) / 1000);
    var pct = Math.max(0, Math.min(1, remaining / totalDuration));
    progressEl.style.transition = instant ? "none" : "transform 1s linear";
  ...
  function tick() {
    ...
    if (remaining <= 0) {
      ...
      saveCachedLock("dirasa_login_lock", null, 0);
      return;
    }
    textEl.textContent = "محاولات كثيرة، حاول بعد " + remaining + " ثانية";
    setProgress(false);
  }
  tick();
  _loginLockInterval = setInterval(tick, 1000);
}

// طبقة تحقق إضافية من طرف السيرفر (Edge Function) قبل إنشاء الحساب —
// تحمي من أي محاولة تتجاوز التحقق المحلي في المتصفح مباشرة.
// إذا تعذر الوصول للدالة (شبكة/عدم نشرها بعد)، نكمل بالتحقق المحلي
// بدل ما نمنع تسجيل مستخدم حقيقي بسبب مشكلة فينا.
async function validateRegistrationServerSide(payload) {
  try {
    const res = await fetch(
      SUPABASE_URL + "/functions/v1/validate-registration",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    console.warn(
      "validateRegistrationServerSide: تعذر التحقق من طرف السيرفر، تابعنا بالتحقق المحلي فقط",
      err,
    );
    return { ok: true };
  }
...
async function handleAuth(type) {
  if (type === "register") {
    ...
    // تحقق من كل الحقول دفعة واحدة، وليس حقلاً بعد حقل
    var regErrors = {};
    var nameErr = validateFullName(name);
    if (nameErr) regErrors.regName = nameErr;
    if (!school) regErrors.regSchool = "أدخل اسم مدرستك أو جامعتك";
    else {
      var schoolErr = validateInstitutionName(school);
      if (schoolErr) regErrors.regSchool = schoolErr;
    }
    if (!email) regErrors.regEmail = "أدخل بريدك الإلكتروني";
    else if (email.length > MAX_EMAIL_LEN)
      regErrors.regEmail =
        "البريد الإلكتروني طويل جداً، الحد الأقصى " + MAX_EMAIL_LEN + " حرفاً";
    else if (!validateEmail(email))
      regErrors.regEmail = "صيغة البريد الإلكتروني غير صحيحة";
    else if (isDisposableEmail(email))
      regErrors.regEmail =
        "هذا النوع من الإيميلات المؤقتة غير مقبول، استعمل بريدك الحقيقي";
    var passErr = validatePassSecurity(pass);
    if (passErr) regErrors.regPass = passErr;

    if (Object.keys(regErrors).length > 0) {
      Object.keys(regErrors).forEach(function (id) {
        showFieldError(id, regErrors[id]);
      });
      return;
    }

    var serverCheck = await validateRegistrationServerSide({
      name: name,
      school: school,
      email: email,
      password: pass,
    ...
    if (serverCheck && serverCheck.ok === false) {
      var fieldErrs = serverCheck.errors || {};
      var regIdMap = {
        name: "regName",
        school: "regSchool",
        email: "regEmail",
      ...
      Object.keys(fieldErrs).forEach(function (key) {
        var id = regIdMap[key];
        if (id) showFieldError(id, fieldErrs[key]);
      });
      return;
    }
    ...
    if (authError) {
      if (authError.message.includes("already registered")) {
        return showFieldError(
          "regEmail",
          "هذا الإيميل مسجّل من قبل، جرّب تسجيل الدخول بدل ذلك",
        );
      }
    ...
    if (Date.now() < _loginLockUntil) {
      applyLoginLock(_loginLockCache.scope, (_loginLockUntil - Date.now()) / 1000);
      return;
    }
    ...
    var loginErrors = {};
    if (!email) loginErrors.email = "أدخل بريدك الإلكتروني";
    if (!pass) loginErrors.pass = "أدخل كلمة المرور";
    if (Object.keys(loginErrors).length > 0) {
      Object.keys(loginErrors).forEach(function (id) {
        showFieldError(id, loginErrors[id]);
      });
      return;
    }

    // تحقق حقيقي من السيرفر قبل أي محاولة دخول فعلية — سلم الإيميل +
    // سلم الـ IP، الاثنين مستقلين (راجع auth-attempt-guard)
    var check = await checkAuthAttempt("login", email);
    if (!check.allowed) {
      applyLoginLock(check.lock_scope, check.remaining_seconds);
      return;
    }
    ...
    if (authError) {
      var outcome = await reportAuthOutcome("login", email, false);
      if (outcome.locked) {
        applyLoginLock(outcome.lock_scope, outcome.remaining_seconds);
        return;
      }
      ...
      if (authError.message.includes("Invalid login")) {
        // رسالة عامة بالنية: تحديد أي حقل بالضبط فيه الخطأ يفتح ثغرة
        // اكتشاف حسابات مسجّلة (Account Enumeration)
        shakeFieldOnly("email");
        showFieldError(
          "pass",
    ...
    // وصلنا هنا يعني الإيميل وكلمة المرور صحيحين فعلاً — دابا بس
    // نبان "جاري تسجيل الدخول"، عكس قبل كانت تبان حتى مع بيانات
    // خاطئة (كانت تُعرض قبل ما نعرف نتيجة signInWithPassword)
    toast("جاري تسجيل الدخول...", "info");
    reportAuthOutcome("login", email, true);
    saveCachedLock("dirasa_login_lock", null, 0);
  ...
  setTimeout(function () {
    ...
    if (skeleton) {
      ...
      setTimeout(function () {
        ...
        var _sb = document.getElementById("searchGroup");
...
function cancelPomoSettings() {
  ...
  toast("تم إلغاء الإعدادات، لم يُحفظ أي تغيير", "info");
}
...
function handleModeSwitch() {
  ...
  if (pomoMode === "study") {
    ...
    sendPomoNotification(
      "🚀 دورة جديدة!",
      "انتهت استراحتك وبدأت دورة جديدة، يمكنك ذلك!",
    );
  }
  updateModeIndicator();
  updateDots();
  ...
  if (pomoMode !== "study" || autoPomoEnabled) {
    ...
    toast("انتهت الاستراحة، اضغط 'ابدأ الآن' للمتابعة 🎯", "info");
  }
}
...
function toggleChill(type) {
  ...
  const audio = new Audio(resolveAudioPath(SOUND_FILES[type], "focus"));
  audio.loop = true;
  audio.volume = 0;
  const playPromise = audio.play();
  if (playPromise !== undefined) {
...
var _focusBtnDefaultHTML = null;
var _focusBtnExitHTML =
  '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align: middle; margin-left: 5px"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>خروج من التركيز';

window.toggleFocusMode = function () {
  _focusMode = !_focusMode;
  document.body.classList.toggle("focus-mode", _focusMode);
  var btn = document.getElementById("focusModeToggle");
  if (btn && _focusBtnDefaultHTML === null) _focusBtnDefaultHTML = btn.innerHTML;
  if (_focusMode) {
    if (btn) btn.innerHTML = _focusBtnExitHTML;
    window.scrollTo({
      top: 0,
      behavior: "smooth",
    });
    ...
    if (btn) btn.innerHTML = _focusBtnDefaultHTML;
...
window.addItem = function (type) {
  ...
  if (val.length > 500) {
    toast("الحد الأقصى لطول المهمة هو 500 حرف", "error");
    return;
  }
  if (user.data.todos.length >= 30) {
    toast("وصلت للحد الأقصى (30 مهمة). احذف مهمة لإضافة أخرى", "error");
    return;
  }
  var priority =
  ...
  inp.value = "";
  updateTodoCharCount();
  autoGrowTextarea(inp, getTextareaLinesHeight(inp, 2));
  saveUser();
  renderList("todo");
  inp.focus();
};
...
function handleTodoAddKeydown(e) {
  if (e.key !== "Enter") return;
  // Shift+Enter: always a new line (desktop and mobile).
  // Plain Enter on desktop: submit the task.
  // Plain Enter on touch devices: a new line too (no keyboard shortcut to
  // submit on mobile — submission there happens via the "+" button).
  if (e.shiftKey || isTouchDevice()) return;
  e.preventDefault();
  addItem("todo");
}

function handleTodoAddInput() {
  updateTodoCharCount();
  var inp = document.getElementById("todoInp");
  autoGrowTextarea(inp, getTextareaLinesHeight(inp, 2));
}
...
function updateTodoCharCount() {
  var inp = document.getElementById("todoInp");
  var counter = document.getElementById("todoCharCount");
...
window.initDashboard = function () {
  ...
  setTimeout(function () {
    ...
    checkOnboarding();
    var sBtn = document.getElementById("searchGroup");
...
// BUG FIX: الحذف الفردي القديم كان يمرر نص المهمة داخل سمة onclick كنص JS خام،
// فأي علامة اقتباس مفردة ' داخل نص المهمة كانت تكسر onclick بالكامل (يعمل أحيانًا
// ويفشل أحيانًا حسب محتوى كل مهمة). هذه النسخة تمرر idx فقط (رقم آمن دائمًا)
// وتقرأ نص المهمة من user.data.todos داخليًا، فتُلغي المشكلة نهائيًا لأي نص.
function removeTodoItem(idx) {
  var t = user.data.todos[idx];
  var fullText = t ? t.text : "";
  // Show only a short preview in the confirm dialog — a long task's full
  // text could visually break that popup's layout.
  var preview =
    fullText.length > 10 ? fullText.slice(0, 10) + "..." : fullText;
  removeItem("todo", idx, preview);
}

var _todoDialogIdx = null;
var TODO_TRUNCATE_LENGTH = 150;
var TODO_TRUNCATE_LINES = 3;

// ── Unified Task Dialog (view mode + edit mode share one popup) ──
function openTodoViewDialog(idx) {
  var item = user.data.todos[idx];
  var box = document.getElementById("todoDialogViewBody");
  var dlg = document.getElementById("todoDialog");
  if (!item || !box || !dlg) return;
  _todoDialogIdx = idx;
...
// Opens the full-text view only if the text is actually cut off:
// either JS already shortened it (wasTruncated, same on every screen size),
// or — mobile only — the browser's own 3-line CSS clamp is visually clipping
// it right now even though JS considered it short enough (scrollHeight >
// clientHeight is a live, native browser measurement, no guessing involved).
// On desktop wasTruncated is the only way this can ever be true, since no
// CSS clamp is applied there — so desktop behavior is 100% unchanged.
function handleTodoTextClick(e, idx, wasTruncated) {
  var el = e.currentTarget;
  var visuallyClipped = el && el.scrollHeight > el.clientHeight;
  if (wasTruncated || visuallyClipped) {
    openTodoViewDialog(idx);
  }
}
...
// Called from the pencil icon in the dialog header while already viewing a task.
function switchToEditMode() {
  if (_todoDialogIdx === null) return;
  enterTodoEditMode();
}

function enterTodoEditMode() {
  var item = user.data.todos[_todoDialogIdx];
  var ta = document.getElementById("todoEditTextarea");
  if (!item || !ta) return;
  ta.value = item.text;
  ta.style.height = "auto";
  setTodoDialogMode("edit");
  updateTodoEditCharCount();
  setTimeout(function () {
    ta.focus();
    ta.select();
    autoGrowTextarea(ta, 230);
  }, 0);
}

function setTodoDialogMode(mode) {
  var title = document.getElementById("todoDialogTitle");
  var editHeaderBtn = document.getElementById("todoDialogEditBtn");
  var viewBody = document.getElementById("todoDialogViewBody");
  var editBody = document.getElementById("todoDialogEditBody");
  if (!title || !editHeaderBtn || !viewBody || !editBody) return;
  if (mode === "edit") {
...
// Exits edit mode (discarding unsaved changes) and closes the popup.
function cancelEditTask() {
  closeTodoDialog();
}

function handleTodoEditKeydown(e) {
  if (e.key === "Escape") {
    e.preventDefault();
    cancelEditTask();
  }
...
function handleTodoEditInput() {
  updateTodoEditCharCount();
  autoGrowTextarea(document.getElementById("todoEditTextarea"), 230);
}

// Generic auto-grow (used by both the add-task field and the edit popup)
// Computes the pixel height of N lines for a given textarea, based on its
// OWN actual computed font metrics — proportional across any screen size,
// font scaling, zoom level, or breakpoint, instead of a hardcoded px guess.
function getTextareaLinesHeight(el, lines) {
  ...
  return Math.ceil(
    lineHeight * lines + paddingTop + paddingBottom + borderTop + borderBottom,
  );
}

// BUG FIX: overflow-y:auto alone could flash a scrollbar even at 1 line due to
// sub-pixel scrollHeight rounding. We now keep it hidden until content truly
// exceeds maxHeight (i.e. only once a real 3rd line appears).
...
function updateTodoEditCharCount() {
  var ta = document.getElementById("todoEditTextarea");
  var counter = document.getElementById("todoEditCharCount");
...
function confirmEditTask() {
  var idx = _todoDialogIdx;
  if (idx === null) return;
  var ta = document.getElementById("todoEditTextarea");
  if (!ta) return;
  var newVal = ta.value.trim();
  if (!newVal) {
    toast("لا يمكن حفظ مهمة فارغة", "error");
    return;
  }
  ...
  user.data.todos[idx].text = newVal;
  closeTodoDialog();
  saveUser();
  renderList("todo");
...
window.renderList = function (type) {
  ...
  list.innerHTML = sorted
    .map(function (s) {
      ...
      var isTruncated =
        textLines.length > TODO_TRUNCATE_LINES ||
        item.text.length > TODO_TRUNCATE_LENGTH;
      var displayText = item.text;
      if (isTruncated) {
        displayText = textLines.slice(0, TODO_TRUNCATE_LINES).join("\n");
        if (displayText.length > TODO_TRUNCATE_LENGTH) {
          displayText = displayText.slice(0, TODO_TRUNCATE_LENGTH);
        }
        displayText += "…";
      }
      return (
        ...
        (isTruncated ? " todo-text-truncated" : "") +
        '"' +
        (isTruncated
          ? ' onclick="handleTodoTextClick(event,' +
            idx +
            ',true)" role="button" tabindex="0"'
          : ' onclick="handleTodoTextClick(event,' + idx + ',false)"') +
        ' style="font-size:13.5px;font-family:var(--font-body);white-space:pre-wrap;' +
        (item.completed
        ...
        "</span>" +
        // Edit
        '<button class="todo-edit-btn" onclick="startEditTask(' +
        idx +
        ')" aria-label="تعديل المهمة">' +
        '<svg viewBox="0 0 512 512" width="15" height="15" class="todo-icon-svg"><path d="M410.3 231l11.3-11.3-33.9-33.9-62.1-62.1L291.7 89.8l-11.3 11.3-22.6 22.6L58.6 322.9c-10.4 10.4-18 23.3-22.2 37.4L1 480.7c-2.5 8.4-.2 17.5 6.1 23.7s15.3 8.5 23.7 6.1l120.3-35.4c14.1-4.2 27-11.8 37.4-22.2L387.7 253.7 410.3 231zM160 399.4l-9.1 22.7c-4 3.1-8.5 5.4-13.3 6.9L59.4 452l23-78.1c1.4-4.9 3.8-9.4 6.9-13.3l22.7-9.1v32c0 8.8 7.2 16 16 16h32zM362.7 18.7L348.3 33.2 325.7 55.8 314.3 67.1l33.9 33.9 62.1 62.1 33.9 33.9 11.3-11.3 22.6-22.6 14.5-14.5c25-25 25-65.5 0-90.5L453.3 18.7c-25-25-65.5-25-90.5 0zm-47.4 168l-144 144c-6.2 6.2-16.4 6.2-22.6 0s-6.2-16.4 0-22.6l144-144c6.2-6.2 16.4-6.2 22.6 0s6.2 16.4 0 22.6z"></path></svg>' +
        "</button>" +
        // Delete
        '<button class="todo-del-btn" onclick="removeTodoItem(' +
        idx +
        ')" aria-label="حذف المهمة">' +
        '<svg viewBox="0 0 15 17.5" height="17.5" width="15" xmlns="http://www.w3.org/2000/svg" class="todo-icon-svg"><path transform="translate(-2.5 -1.25)" d="M15,18.75H5A1.251,1.251,0,0,1,3.75,17.5V5H2.5V3.75h15V5H16.25V17.5A1.251,1.251,0,0,1,15,18.75ZM5,5V17.5H15V5Zm7.5,10H11.25V7.5H12.5V15ZM8.75,15H7.5V7.5H8.75V15ZM12.5,2.5h-5V1.25h5V2.5Z"></path></svg>' +
        "</button>" +
        "</div>"
      );
    })
...
window.bpSkipAdvices = function () {
  bpClosePanel();
  toast("رجعت للاستراحة العادية، استرح قليلاً ☕", "info");
};
  ...
  try {
    ...
    if (!currentUser) {
      if (typeof toast === "function")
        toast("لم يتم التعرف على حسابك، سجّل الدخول أولاً", "error");
      closeDeleteConfirm();
      return;
    }
